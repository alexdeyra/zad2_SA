    ...
    public partial class Form1 : Form
    {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            if (!(name == "" || number == ""))
            {
                ...
                if (check)
                {
                    // Используем метод для поиска контакта по номеру телефона
                    bool contactExists = pb.GetAllContacts().Any(contact => contact.Phone == number);
                    if (contactExists) // Если контакт существует
                    {
                        MessageBox.Show("Контакт с таким номером телефона уже существует."); // Сообщение об ошибке
                        check = false;
                    }
                }
